...
<head>
    <title>Aviso Prévio - Impressão - Link Contábil</title>
...
<body>
    ...
    <section class="container documento">
        ...
        <h3 align="center"><strong>AVISO PRÉVIO DE DISPENSA</strong></h3>
        <h5 align="center"><small>*Artigo 487 da Consolidação das Leis do Trabalho (CLT)</small></h5><br />

        <p>Prezado(a) Sr.(a): <?php $nome_funcionario = $_POST['nome_funcionario']; echo "$nome_funcionario" ?></p>

        <p>Vimos por meio desta comunicá-lo(a) que após o dia <strong><?php $data = $_POST['data']; $data = DateTime::createFromFormat('d/m/Y', $data); $data->add(new DateInterval('P29D')); echo $data->format('d/m/Y');?></strong> não mais utilizaremos seus serviços.</p>

        <p>De acordo com a Consolidação das Leis do Trabalho efetuaremos o pagamento de suas verbas rescisórias no dia <strong><?php $data = $_POST['data']; $data = DateTime::createFromFormat('d/m/Y', $data); $data->add(new DateInterval('P30D')); echo $data->format('d/m/Y');?> </strong> ou no <strong>1º</strong> dia útil subsequente e  solicitamos desde já sua presença neste dia, para os procedimentos legais, em cumprimento a Lei, portando a Carteira de Trabalho e Previdência Social, bem como exame médico demissional, de acordo com o Artigo 168 da C.L.T., cujo custo é de nossa responsabilidade.</p>

        <p>Comunicamos que o não comparecimento do(a) Sr(a). neste dia isenta a empresa de qualquer ônus que deste fato advir.</p>

        <p>Como o motivo do desligamento é DISPENSA SEM JUSTA CAUSA, o(a) Sr(a). pode optar por uma destas formas de cumprimento do Aviso Prévio, que será de <b>30</b> dias conforme a C.L.T.. Solicitamos que faça sua escolha:</p>

        <p>( ) Trabalhar em horário normal, durante os 23 primeiros dias,  até o dia <b><?php $data = $_POST['data']; $data = DateTime::createFromFormat('d/m/Y', $data); $data->add(new DateInterval('P22D')); echo $data->format('d/m/Y');?></b>, desobrigando-se do cumprimento dos 7 dias restantes, sem prejuízo da remuneração dos mesmos.</p>

        <p>( ) Ingressar no trabalho 02 duas horas após o início de seu horário de trabalho, sem prejuízo de remuneração das mesmas.</p>

        <p>( ) Deixar o trabalho 02 duas horas antes do término de seu horário de trabalho, sem prejuízo de remuneração das mesmas.</p>
